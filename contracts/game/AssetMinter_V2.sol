...
import "./WarrantIssuer_V2.sol";

contract AssetMinter_V2 is
    Context,
    Pausable,
    AccessControl
{
    ...
    address public _warrantIssuerAddr;
    ...
    function init(
        ...
        _warrantIssuerAddr = warrantIssuerAddr;
    }
    ...
    function mintPackage(uint32 packageId, address userAddr) external whenNotPaused {
        require(hasRole(MINTER_ROLE, _msgSender()), "AssetMinter: must have minter role");

        PackageInfo storage pi = _packages[packageId];

        require(pi.totalCount > 0, "AssetMinter: package not exist");
        require(pi.totalCount > pi.mintedCount, "AssetMinter: insufficient package");
        ...
        for(uint i=0; i< pi.portIDs.length; ++i){
            WarrantIssuer_V2(_warrantIssuerAddr).mint_MTTWarrantByService(pi.portIDs[i], userAddr);
        }
