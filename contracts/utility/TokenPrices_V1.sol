    ...
    function getERC20TokenUSDPrice(address tokenAddr) public override view returns (uint256) {
        ...
        if(defiPool.poolType != 0){
            ...
            if(tokenAddr != address(0)){
                amount = 1*(10**ERC20(tokenAddr).decimals());
            }

            // get price from defi pool
            if(defiPool.poolType == DefiPoolType_UniswapV2) {
                if(defiPool.tokenIndex == 0){
                    return _univ2_getTokenPrice_0(defiPool.poolAddr, amount);
                }
                else {
                    return _univ2_getTokenPrice_1(defiPool.poolAddr, amount);
                }
            }
            else if(defiPool.poolType == DefiPoolType_UniswapV3){
                if(defiPool.tokenIndex == 0){
                    return _univ3_getTokenPrice_0(defiPool.poolAddr, amount);
                }
                else {
                    return _univ3_getTokenPrice_1(defiPool.poolAddr, amount);
                }
            }
            else {
